...
function formatTime(minutes, seconds) {
  const strMimutes = String(minutes).padStart(2, '0');
  const strSeconds = String(seconds).padStart(2, '0');
  return strMimutes.concat(':').concat(strSeconds);
}
...
function findLongestWord(sentence) {
  ...
  let largest = sentenceWords[0];
  for (let i = 1; i < sentenceWords.length; i += 1) {
    if (largest.length < sentenceWords[i].length) {
      largest = sentenceWords[i];
    }
  }
  return largest;
}
